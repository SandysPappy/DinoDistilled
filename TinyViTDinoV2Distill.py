    ...
    for epoch in range(FLAGS.num_epochs):
        ...
        if best_val_loss is None:
            ...
            if current_test_loss<best_val_loss:
                torch.save(student_model.state_dict(), f"{FLAGS.log_dir}/output/v2student_model_tinyvit_best_test_loss.pth")

        print(f"EPOCH[{epoch}] Train loss: {running_loss / len(data_loader_train)} Test loss: {test_loss / len(data_loader_test)}")

    os.makedirs("output", exist_ok=True)
    torch.save(student_model.state_dict(), f"{FLAGS.log_dir}/output/v2student_model_tinyvit.pth")
